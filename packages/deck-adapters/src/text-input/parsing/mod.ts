...
import { XTextCommand } from "./extended-commands.ts";
import {
  appendErrorMessageWithCommandUsage,
  findCommandUsageHeadsByName,
} from "../usage/commands/mod.ts";

const MAX_SAFE_INTEGER = 9_007_199_254_740_991;

export type TextInputCommand =
  | Command
  | { type: "x_text"; payload: XTextCommand };
...
/**
 * 解析通常的文本输入。也即完整形式是 “<根前缀><剩余>” 的输入。
 *
 * @param input 对应上述中的 “<剩余>”。如果以 “：” 开头，那就是针对指定卡<wbr/>
 * 组的命令，不然就是针对整个插件的命令。
 * @param opts
 */
export function parseTextInputRegular(input: string, opts: {
  rootPrefix: string;
}): CommandParseResult {
...
/**
 * 已知是针对卡组的命令及所针对卡组的名称，解析文本输入的剩余部分。
 *
 * @param input 对应 “卡组：<卡组名> <剩余>” 中的 “<剩余>”。
 * @param opts
 */
export function parseTextInputForDeck(input: string, opts: {
  rootPrefix: string;
  deckName: DeckName;
}): ["ok", Command] | ["error", string, any?] | ["ignore"] {
  return parseDeckCommand(input, opts.deckName, {
    rootPrefix: opts.rootPrefix,
  });
}
...
export function parseCommand(
  ...
  if (!/^[:：]/.test(input)) { // 不以冒号开头，说明是和单独卡组无关的插件级别命令
    ...
    {
      const TYPE = "x_text" as const;

      const result = tryParseXTextCommand(cmd, rest);
      switch (result[0]) {
        case "ok":
...
function tryParseXTextCommand(cmd: string, rest: string | null):
  | ["ok", XTextCommand]
  | ["not_found"] {
  if (rest) {
